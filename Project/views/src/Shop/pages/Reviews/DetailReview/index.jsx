...
function DetailReview() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const resDetailExport = await detailExportInvoiceAPI.getDetailExportInvoice(params.idhdx);
  ...
  const uploadImage = async (e) => {
    ...
    if (imageNumber <= 3) {
      ...
      // enqueueSnackbar("Image is max 3", {
      //   variant: "error",
      //   autoHideDuration: 2000,
      // });
      console.log("NO");
  ...
  return (
    <div className="w-[50%] my-6 mx-auto">
      <div className="flex justify-between my-4">
        ...
        <p className="text-green-700 font-bold">{exportInvoice[0]?.trang_thai}</p>
      </div>
      <div>
        {detailExportInvoice?.map(({id_sp, ten_sp, hinh_anh, so_luong_xuat, id_ms, id_kt, ten_ms, ten_kt}, idx) => (
        ...
        >
          <Box sx={style}>
            <div className="relative mb-2 flex justify-between gap-5 items-center">
              <img className="block w-[10%] align-middle rounded-lg" src={product[0]?.hinh_anh} alt="" />
              <span className="w-[70%] text-[20px] font-bold">{product[0]?.ten_sp}</span>
              <span className="w-[20%] text-right">Số lượng: {product[0]?.so_luong_xuat}</span>
              <div className="absolute flex gap-5 left-[12.5%] top-[75%] text-slate-500 text-[14px]">
            ...
            {checkReview.length === 0 ? (
              ...
              <>
                <div>
                  <Rating className="my-3" name="read-only" size="large" value={checkReview[0]?.so_sao} readOnly />
                  <div className="flex gap-5">{renderImageRV}</div>
                  <p className="mt-5">Đánh giá: {checkReview[0]?.noi_dung_dg}</p>
                  <button
