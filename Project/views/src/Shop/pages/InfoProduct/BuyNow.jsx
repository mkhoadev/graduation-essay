...
function BuyNow() {
    ...
    (async () => {
      ...
      if (listBuy.length !== 0) {
        for (let i = 0; i < listBuy.length; i++) {
          a =
            a + listBuy[i]?.so_luong_xuat * (listBuy[i]?.gia_ban - (listBuy[i]?.gia_ban * listBuy[i]?.giam_gia) / 100);
        }
        setSumPrice(a);
      }
    })();
  ...
  const renderProduct =
    listBuy?.length > 0 ? (
      listBuy?.map(({ten_sp, gia_ban, giam_gia, hinh_anh, so_luong_xuat, ten_ms, ten_kt}, idx) => (
  ...
  const selectProvince = async (code) => {
    ...
    setDistrict(data_province);
    setProvinceName(data_province.data.name);
    setCount((e) => e + 1);
  };

  const selectDistrict = async (code) => {
    const data_district = await provinceAPI.getDistricts(code);
    setWards(data_district);
    setDistrictName(data_district.data.name);
    setCount((e) => e + 1);
  };

  const selectWard = async (code) => {
    const data_ward = await provinceAPI.getWards(code);
    setWardName(data_ward.data.name);
  };
    ...
    <div className="flex gap-5 w-[80%] mx-auto mt-[50px]">
        ...
        <div>
          <div className="bg-slate-100">
            <div className="flex justify-between py-2 px-4 bg-slate-200">
              ...
              <p onClick={() => handleSettingAddress()} className="cursor-pointer text-sky-600">
                Chọn địa chỉ
              </p>
              <Modal
                open={openAddress}
              ...
              >
                <Box sx={styleAddress}>
                  <div className="flex justify-between">
                    ...
                    <p onClick={() => addAddress()} className="text-sky-900 cursor-pointer">
                      Thêm địa chỉ mới
                    </p>
                  </div>
                  <div className="mt-6">
              ...
              >
                <Box sx={styleFormAddress}>
                  <p className="mb-4 text-[18px] font-medium">Thêm địa chỉ nhận hàng mới</p>
                  <form onSubmit={handleSubmit((data) => submitAddAddress(data))}>
                    <div className="flex gap-5">
                      ...
                      <div className="w-1/2">
                        ...
                        <div className="mt-4">
                          ...
                          >
                            <option value=""></option>
                            {district?.data?.districts?.map(({name, code}, idx) => (
                              <option key={idx} value={code}>
                                {name}
                              </option>
                            ))}
                        ...
                        <div className="mt-4">
                          ...
                          >
                            <option value=""></option>
                            {wards?.data?.wards?.map(({name, code}, idx) => (
                              <option key={idx} value={code}>
                                {name}
                              </option>
                            ))}
      ...
      <div className="w-[30%]">
        <div className="p-4 bg-slate-100 rounded-lg">
          ...
          <div className="mt-4">
            <button onClick={() => payoffline()} className="w-full p-2 text-white bg-orange-400 rounded-md">
              Thanh toán khi nhận hàng
            </button>
            <button onClick={() => payonline()} className="w-full p-2 text-white mt-4 bg-sky-500 rounded-md">
