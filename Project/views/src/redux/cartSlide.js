  ...
  reducers: {
    addtoCart(state, action) {
      ...
      const index = state.cartItem.findIndex(
        (x) => !!(x.id_sp === newItem.id_sp && x.id_ms === newItem.id_ms && x.id_kt === newItem.id_kt),
      );
      if (index >= 0) {
    ...
    removeFromCart(state, action) {
      ...
      state.cartItem = state.cartItem.filter(
        (x) => !(x.id_sp === idRemove.id_sp && x.id_ms === idRemove.id_ms && x.id_kt === idRemove.id_kt),
      );
      ...
      localStorage.setItem(
        "cart",
        JSON.stringify(
          a.filter((x) => !(x.id_sp === idRemove.id_sp && x.id_ms === idRemove.id_ms && x.id_kt === idRemove.id_kt)),
        ),
      );
    },
